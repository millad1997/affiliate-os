// src/lib/brand-config-read.isolation.check.ts
// Cross-tenant isolation positive control for getBrandConfig.
// Run (server-only module, so the react-server condition is required):
//   npx tsx --conditions=react-server --env-file=.env.local \
//     src/lib/brand-config-read.isolation.check.ts <brandB> <ownerB> <brandA> <ownerA>
...
import { getBrandConfig } from "./brand-config-read";
...
async function main(): Promise<void> {
  ...
  const r1 = await getBrandConfig(brandB, ownerB);
  assert("owner B reads B's brand → ok", r1.ok === true);

  const r2 = await getBrandConfig(brandB, ownerA);
  assert("non-owner A denied B's brand → not_found", r2.ok === false && r2.reason === "not_found");

  const r3 = await getBrandConfig(brandA, ownerA);
  assert("owner A reads A's brand → ok", r3.ok === true);

  const r4 = await getBrandConfig(brandA, ownerB);
